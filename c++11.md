...
#### auto -- deduction of a type from an initializer:
    template<class T, class U> void multiply(
      const vector<T>& vt, const vector<U>& vu) {
        auto tmp = vt[i]*vu[i];
    }
...
#### inherited constructors:
    struct B1 { B1(int) {} };
    struct D1 : B1 { using B1::B1; }; // implicitly declares D1(int)
...
- Not VLA  
- \__func\__ a macro of the lexically current function  
- \__STDC_HOSTED\__  
...
#### locks:
    ...
    // make a lock, but don't acquire the mutex
